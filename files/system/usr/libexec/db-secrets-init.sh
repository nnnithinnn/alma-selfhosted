...
# podman secrets, owned by the `nithin` user.
#
# Runs once, early in nithin's (lingered) user systemd session, BEFORE the
# postgres/valkey quadlets start (they order After=db-secrets-init.service).
# The secrets are stored in nithin's persistent container storage under /var,
...
# Nextcloud's initial admin password (user `nithin`). Consumed via
