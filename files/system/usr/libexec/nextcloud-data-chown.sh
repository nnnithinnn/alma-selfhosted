...
#   Nextcloud runs ROOTLESS as `nithin`, whose subordinate UID range is
...
#   (nithin cannot chown to a uid it merely "owns" via the map, and rootless
...
# Resolve nithin's subuid base from /etc/subuid (login name or uid 1000).
subline="$(awk -F: '$1=="nithin" || $1=="1000" {print; exit}' /etc/subuid)"
if [ -z "${subline:-}" ]; then
    log "ERROR: no subuid range for nithin in /etc/subuid; refusing."
    exit 1
fi
