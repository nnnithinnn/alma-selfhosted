...
# selfhosted-configure.service reads /etc/selfhosted/config.env (injected at
# install time by the kickstart) and applies @@TOKEN@@ substitution to produce
# live config files from the templates in /usr/lib/selfhosted/templates/.
# It runs very early (sysinit.target, before sysusers + tmpfiles) so the user
# declaration and all container configs are in place before any service starts.
chmod 0755 /usr/lib/selfhosted/configure
systemctl enable selfhosted-configure.service

### selfhosted CLI #############################################################
# /usr/bin/selfhosted {status,upgrade} — operational helper for the app user.
chmod 0755 /usr/bin/selfhosted
...
#   - files/services/usr/libexec/nextcloud-data-init.sh
#   - files/services/usr/lib/systemd/system/nextcloud-data-init.service
#   - files/services/etc/systemd/system/var-mnt-nextcloud.mount
#   - files/services/usr/lib/tmpfiles.d/nextcloud.conf
...
#   - files/services/usr/libexec/nextcloud-data-chown.sh
#   - files/services/usr/lib/systemd/system/nextcloud-data-chown.service
...
#   - files/services/usr/libexec/nextcloud-apps-init.sh
#   - files/services/etc/systemd/user/nextcloud-apps-init.service
...
#   - files/services/usr/libexec/db-secrets-init.sh
#   - files/services/etc/systemd/user/db-secrets-init.service
